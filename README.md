...
**3. Coordinates → `out/coords.js`.** `src/compare.py` projects the LLM scores into Inglehart–Welzel space using the saved weights and combines them with the country centroids → columns `country, region, x, y`. For each LLM it also emits an **80% confidence ellipse of the mean** (`ea, eb, etheta, en` = semi-axes, angle, n) via Hotelling's T², drawn under the point in `index.html`. It scales with that model's own response count `n` (fewer responses → larger ellipse), so it shows how well-pinned the centroid is versus a country's. Set the level with `--conf` (e.g. `--conf 0.9`); higher = larger. Countries get no ellipse.
