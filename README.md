...
<img height="600" alt="image" src="https://github.com/user-attachments/assets/29f4039d-766f-4dcf-93ca-73f331d8ee81" />
...
Prompts live under `data/prompts/`, with the **question language and the persona chosen independently** so any style can be combined freely. Questions are `questions_<lang>.csv` (`scale,prompt`), selected by an experiment's `language` field (default `EN`) via the `QLANG` table in `src/run_experiments.py` (add a language by dropping in its CSV and one `QLANG` row). Personas are `respondents_<persona>.csv` (`variant,respondent_descriptor,batch`), selected by the `persona` field, which **defaults to the generic set for the question language** (e.g. `language: NO` → `respondents_no.csv`). Role/native sets follow `respondents_<persona-lang>_<role>.csv` — e.g. `en_no_role` (Norwegian role, in English), `no_no_role` (Norwegian role, in Norwegian), `ka_ka_role` (Georgian, in Georgian) — so English questions can carry a Georgian-role persona, or Norwegian questions a Norwegian-role persona in Norwegian. Persona phrasings are tagged with the `batch` (version) they were introduced in; batches are cumulative (`v2` includes `v1`). `config/experiments.json` is the single source of truth for the `language`/`persona`/`batch` of each experiment; `compare.py --language <L> --persona <P>` selects one cell (persona defaults to the generic set, so the main map stays generic).
